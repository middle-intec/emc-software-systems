...
<button type="button" id="btn_boduyet" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span> Bỏ duyệt</button>
	...
	echo '
<button type="button" id="tl_duyet" class="btn btn-info"><span class="glyphicon glyphicon-ok"></span> Duyệt</button>  
<button type="button" id="tl_boduyet" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span> Bỏ duyệt</button>
	';
}
?>  
</div>
		...
		<table class="table table-bordered">
			<thead>
							<tr class="success">
						  		...
						  		<th>Kết quả HĐCT</th>
						  		<th>PT</th>
						  		<th>TL<br/><input type="checkbox" id="checkAlltl"/></th>
						  		<th>TGĐ<br/><input type="checkbox" <?php if($_SESSION['level']!=1){echo 'disabled';}?> id="checkAlltgd"/></th>
			...
			<tbody>
			        ...
			        <?php if(($row_xemkqkpi["pt"])==1){echo 'checked="checked" ';}
								?></>
			        			</td>
			        			<td>
			        			<?php
				if((($_SESSION['level'])==1)or(($_SESSION['level'])!=5)){
					echo "<input disabled class='checktl' type='checkbox'";
			         if(($row_xemkqkpi["tl"])==1){echo 'checked="checked" ';}echo '/>';
			     }else{
			     	echo "<input class='checktl' name = 'id_kqkpi[]' value = '".$row_xemkqkpi["id_kqkpi"]."' type='checkbox'";
			         if(($row_xemkqkpi["tl"])==1){echo 'checked="checked" ';}echo '/>';
			     }
								?>
			        			 </td>
							...
							<?php
				if(($_SESSION['level'])==1){
					echo "<input class='checktgd' name = 'id_kqkpi[]' value = '".$row_xemkqkpi["id_kqkpi"]."' type='checkbox'";
			         if(($row_xemkqkpi["tgd"])==1){echo 'checked="checked" ';} echo '/>';
			     }else{
			     	echo "<input disabled class='checktgd' type='checkbox'";
			         if(($row_xemkqkpi["tgd"])==1){echo 'checked="checked"';}echo '/>';
			     }
								?></td>
			        			<td width="85px">
...
<div id="dataModal" class="modal fade">  
 ...
 $(document).ready(function(){  
	...
	$('#checkAlltl').change(function() {
			    var checkboxes = $(this).closest('table').find('.checktl');
			    if($(this).is(':checked')) {
	...
	$('#checkAlltgd').change(function() {
			    var checkboxes = $(this).closest('table').find('.checktgd');
			    if($(this).is(':checked')) {
			        checkboxes.prop('checked', true);
			    } else {
			        checkboxes.prop('checked', false);
	...
	$('#btn_duyet').click(function(){  
           ...
           {  
                ...
                {  
                     $.ajax({  
                          url:'data_ajax/duyet_kpis_tgd.php',  
                          method:'POST',  
                          data:{id:id},  
                          success:function()  
                          {  
		...
		$('#btn_boduyet').click(function(){  
           ...
           {  
                ...
                {  
                     $.ajax({  
                          url:'data_ajax/boduyet_kpis_tgd.php',  
                          method:'POST',  
                          data:{id:id},  
                          success:function()  
                          {  
		...
		$('#tl_duyet').click(function(){  
           ...
           {  
                ...
                {  
                     $.ajax({  
                          url:'data_ajax/duyet_kpis_tl.php',  
                          method:'POST',  
                          data:{id:id},  
                          success:function()  
                          {  
		...
		$('#tl_boduyet').click(function(){  
           ...
           {  
                ...
                {  
                     $.ajax({  
                          url:'data_ajax/boduyet_kpis_tl.php',  
                          method:'POST',  
                          data:{id:id},  
                          success:function()  
                          {  
